...
### Test
#### 1. LottoTest
- [x] `toString`이 의도한 형식대로 문자열을 반환하는지 테스트
- [x] `toSet`가 로또 번호를 포함하는 집합을 반환하는지 테스트 

#### 2. LottoViewModelTest
- [x] 입력된 금액에 따른 로또 횟수를 올바르게 계산하는지 테스트
- [x] 생성된 로또 번호가 중복되지 않는지 테스트
- [x] 로또 생성 후 리스너가 정상적으로 호출되는지 테스트

#### 3. InputMoneyViewModelTest
- [x] 정상적인 금액 입력시 리스너가 정상적으로 호출되는지 테스트
- [x] 비정상적인 금액 입력시 에러 리스너가 정상적으로 호출되는지 테스트

#### 4. InputNumberViewModelTest
- [x] 유효한 로또 번호를 입력했을 때 리스너가 정상적으로 호출되는지 테스트
- [x] 유효하지 않은 로또 번호를 입력했을 때 에러 리스너가 정상적으로 호출되는지 테스트
- [x] 유효한 보너스 번호를 입력했을 때 리스너가 정상적으로 호출되는지 테스트
- [x] 로또 번호와 중복된 보너스 번호를 입력했을 때 에러 리스너가 정상저긍로 호출되는지 테스트
- [x] 범위 밖의 보너스 번호를 입력했을 때 에러 리스너가 정상적으로 호출되는지 테스트

#### 5. ResultViewModelTest
- [x] 발행된 로또번호 리스트가 초기화될때 내부 상태가 업데이트 되는지 테스트
- [x] 결과 업데이트시 올바른 문자열을 반환하는지 테스트
- [x] 수익률 계산시 올바른 결과를 반환하는지 테스트