...
enum class Winning(val lottoWinnings: String, val match: Int = 0, val count: Int = 0) {
    ...
    override fun toString() =
        if (lottoWinnings != WINNING_SECOND_PLACE) "${match}개 일치 ($lottoWinnings) - ${count}개"
        else "${match}개 일치, 보너스 볼 일치 ($lottoWinnings) - ${count}개"
}
