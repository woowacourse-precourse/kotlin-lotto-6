...
class Lotto(private val numbers: List<Ball>) {
    ...
    fun isMatchedBonus(bonus: Ball): Boolean = numbers.contains(bonus)
    ...
    companion object {
        ...
        fun create(userInput: String): Lotto {
            val balls = userInput.split(SEPARATOR).map { number -> number.trim().toValidInt() }.toBalls()
            return Lotto(balls)
        }

        fun of(vararg balls: Int) = Lotto(balls.toList().toBalls())
    }
}