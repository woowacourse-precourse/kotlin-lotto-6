...
class LottoModel {
    ...
    fun checkWinningTicket(
        buy_lotto_number: Array<Lotto?>,
        winning_number: List<String>,
        bonus_number: Int,
        size: Int
    ): IntArray {
    ...
    fun chooseWinning(
        ...
        bonus_number: Int,
        size: Int,
        ticket_result: IntArray
    ): IntArray {
        for (ticket in 0..size - 1) {
            var get_ticket_line = getTicketLine(buy_lotto_number, ticket)//선택된 로또의 번호들을 저장하는 함수
            var winning_lotto_num =
                checkLottoWinningAgreement(get_ticket_line, winning_number)//당첨과 같은 번호와 갯수를 저장하는 PAIR 생성
