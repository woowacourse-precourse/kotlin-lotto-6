...
class RateOfReturnTest {
    ...
    fun `당첨금이 0원인 경우) 수익률 0,0 나오는지 확인`() {
        val rateOfReturn = RateOfReturn(lottosMatchCount.result, 10000).getData()
    ...
    fun `7장 구매해서 5천원 당첨된 경우) 소수점 2째자리에서 버리는 Case (71,43 to 71,4)`() {
        ...
        val rateOfReturn = RateOfReturn(lottosMatchCount.result, 7000).getData()
    ...
    fun `30장 구매해서 50만원 당첨된 경우) 소수점 2째자리에서 올림하는 Case (1666,66 to 1666,7)`() {
        ...
        val rateOfReturn = RateOfReturn(lottosMatchCount.result, 30000).getData()
